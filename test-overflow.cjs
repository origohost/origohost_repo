...
const routes = [
  '/', '/about', '/community', '/sponsor', '/events', 
  '/contact', '/register', '/login', '/trust-center', 
  '/privacy', '/terms', '/security', '/code-of-conduct',
  '/academy', '/blog', '/glossary'
];
const devices = [
  ...
  for (const device of devices) {
    ...
    for (const route of routes) {
      ...
      try {
        ...
        if (hasOverflow) {
          console.log(`[OVERFLOW] ${route} on ${device.name}: ${hasOverflow.docWidth} > ${hasOverflow.winWidth}`);
